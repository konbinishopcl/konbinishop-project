...
- **D-05:** `GET /articles` (público) devuelve solo artículos con `status=APPROVED`. `GET /articles` (admin, con query param `?all=true` o header `X-Admin: true`) devuelve todos los estados. Los artículos patrocinados (`userId != null`) se distinguen por el campo `isSponsored: true` en la respuesta (campo derivado, no persistido).
