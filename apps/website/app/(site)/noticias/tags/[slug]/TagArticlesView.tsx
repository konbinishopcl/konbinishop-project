...
export function TagArticlesView({ tag, initialArticles, initialTotal, initialTotalPages }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/articles?${params}`)
      ...
      .catch(() => { if (!cancelled) { setArticles([]); setTotal(0); setTotalPages(1); } })
  ...
  function clearAll() {
    setType("Todos"); setOrigen("Todos"); setPeriod(null); setQ(""); setPage(1);
  }
  ...
  // ── Pop dropdown ───────────────────────────────────────────────────────
  function Pop({ id, label, options, value, onPick, icon, dflt }: {
      ...
      <div style={{ position: "relative" }}>
        ...
        {openPop === id && (
          <div className="menu" style={{ top: 44, left: 0, minWidth: 200 }} onMouseLeave={() => setOpenPop(null)}>
            ...
            {options.map((o) => (
              <button key={o} onClick={() => { onPick(o); setOpenPop(null); }}
    ...
    <main className="container">
      ...
      <div style={{ margin: "8px 0 24px", display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 16 }}>
        <div>
          ...
          <p style={{ color: "var(--ink-2)", margin: 0, maxWidth: "60ch" }}>
            ...
            {" "}· artículos etiquetados con #{tag.name} en Konbini.
          </p>
        </div>
      ...
      <div className="fbar-sticky">
        <div className="fbar-inner">
          ...
          <div className="group">
            <Pop
              id="type" label="Tipo" options={TYPE_OPTIONS} value={type} dflt="Todos"
              onPick={(v) => { setType(v); setPage(1); }}
              icon={<span style={{ display: "inline-flex", marginRight: 6 }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M4 6h16M4 12h12M4 18h8"/></svg></span>}
            />
            <Pop
              id="origen" label="Origen" options={ORIGEN_OPTIONS} value={origen} dflt="Todos"
              onPick={(v) => { setOrigen(v); setPage(1); }}
              icon={<span style={{ display: "inline-flex", marginRight: 6, fontWeight: 700, fontSize: 14 }}>✦</span>}
            />
            {activeCount > 0 && (
          ...
          <div className="group" style={{ marginLeft: "auto" }}>
            <div style={{ position: "relative" }}>
              ...
              {searchOpen && (
                <div className="menu" style={{ top: 44, right: 0, left: "auto", minWidth: 280, padding: 12 }} onMouseLeave={() => setSearchOpen(false)}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--surface-2)", borderRadius: 10, padding: "8px 12px" }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg>
                    <input
                      autoFocus
                      placeholder={`Buscar en #${tag.name}…`}
                      value={q}
                      onChange={(e) => { setQ(e.target.value); setPage(1); }}
                      style={{ flex: 1, padding: 0, background: "transparent", border: 0, outline: "none", fontSize: 14 }}
                    />
                    {q && (
                      <button onClick={() => setQ("")} style={{ padding: 4, borderRadius: 6, display: "flex", alignItems: "center" }}>
            ...
            <div style={{ display: "inline-flex", border: "1px solid var(--line)", borderRadius: 999, padding: 3, background: "var(--surface)" }}>
              <button className={`sel ${view === "grid" ? "on" : ""}`} onClick={() => setView("grid")} title="Grilla" style={{ border: 0, padding: "6px 10px" }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>
              </button>
              <button className={`sel ${view === "list" ? "on" : ""}`} onClick={() => setView("list")} title="Lista" style={{ border: 0, padding: "6px 10px" }}>
            ...
            <Pop
              id="sort" label="Ordenar" options={SORT_OPTIONS} value={sort} dflt="Más recientes"
              onPick={(v) => { setSort(v); setPage(1); }}
              icon={<span style={{ display: "inline-flex", marginRight: 6 }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M3 6h13M3 12h9M3 18h5M17 8V4l3 3M17 16v4l3-3"/></svg></span>}
            />
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="list-grid" style={{ marginTop: 16, marginBottom: 24 }}>
          {filtered.map((a) => (
            <Link key={a.id} className="list-row" href={`/noticias/${a.slug}`}>
              <div className="l-img">
                <div className="pic" style={{ backgroundImage: a.image ? `url(${imageUrl(a.image)})` : "linear-gradient(135deg,#2a1410,#4a1820)" }} />
                <span className="cat-tag">{getCat(a).toUpperCase()}</span>
              </div>
              <div className="l-info">
                <div className="l-date">{formatDate(a.createdAt)} · {readingTime(a.content ?? "")} lectura</div>
                ...
                {a.isSponsored && <span className="pill" style={{ marginTop: 8, fontSize: 10, display: "inline-flex" }}>ARTÍCULO PATROCINADO</span>}
              </div>
              <div />
      ...
      {!loading && total > 0 && (
        <div className="pag-bar">
          ...
          <div className="pag-pages">
            <button onClick={() => setPage((p) => p - 1)} disabled={page === 1} title="Anterior"><ChevL /></button>
            {pageWindows(page, totalPages).map((p, i) =>
              p === "…" ? (
                <span key={`ell-${i}`} className="ell">…</span>
              ) : (
                <button key={p} className={page === p ? "on" : ""} onClick={() => setPage(p as number)}>{p}</button>
              )
            )}
            <button onClick={() => setPage((p) => p + 1)} disabled={page === totalPages} title="Siguiente"><ChevR /></button>
          </div>
        </div>
      )}
    </main>
