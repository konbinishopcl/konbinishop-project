...
const FIELDS: Field[] = [
  ...
  { k: "minDays",     label: "Días mínimos",       type: "number", required: true, placeholder: "10" },
  { k: "maxDays",     label: "Días máximos",       type: "number", required: true, placeholder: "60" },
];
    ...
    <>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div style={{ color: "var(--ink-3)", fontSize: 13 }}>
          Categorías de <strong style={{ color: "var(--ink)" }}>Eventos</strong> · con precio y duración
          ...
          <tbody>
            {paginated.map((c) => (
              <tr key={c.id}>
                ...
                <td style={{ fontFamily: "var(--font-mono)", color: "var(--ink-3)" }}>{(c as ApiEventCategory & { _count?: { events?: number } })._count?.events ?? "—"}</td>
                <td>
                  <div className="row-act">
      ...
      {modal?.type === "create" && (
        <AdminFormModal
          title="Nueva categoría de evento"
          fields={FIELDS}
