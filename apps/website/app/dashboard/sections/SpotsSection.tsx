...
export default function SpotsSection() {
  ...
  return (
    <>
      ...
      {/* Modals */}
      {modal?.type === "approve" && (
        <AdminApproveModal kind="aviso" onClose={close} onApprove={() => {}} />
      )}
      {modal?.type === "reject" && (
        <AdminRejectModal kind="aviso" onClose={close} onReject={() => {}} />
      )}
      {modal?.type === "transfer" && (
      ...
      {modal?.type === "ban" && (
        <ConfirmDialog
          danger
          title="¿Banear aviso?"
          message="El aviso dejará de ser público inmediatamente. El organizador será notificado. Puedes restaurarlo después si fue un error."
